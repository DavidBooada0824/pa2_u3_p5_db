...
public class HabitacionRepoImpl implements IHabitacionRepo {
	...
	public List<Habitacion> seleccionarHabitacionPorNumero(String numero) {
		TypedQuery<Habitacion> query = this.entityManager.createQuery(
				"select h from Habitacion h  inner join h.hotel ha where h.numero=:numero  ", Habitacion.class);
	...
	public List<Habitacion> seleccionarHabitacionPorClase(String clase) {
		TypedQuery<Habitacion> query = this.entityManager.createQuery(
				"select h from Habitacion h  right join h.hotel ha where h.clase=:clase  ", Habitacion.class);
